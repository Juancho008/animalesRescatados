...
export async function handleAnimalImageDownload(env, id, corsHeaders) {
...
/** POST /api/admin/images — subir imagen del animal */
export async function handleAnimalImageUpload(request, env, json) {
  ...
  if (buffer.byteLength > MAX_BYTES) {
    return json({ error: "La imagen no puede superar 3 MB" }, 400);
  }
  ...
  const meta = {
    ...
    filename: (payload.filename || "animal.jpg").toString(),
