...
function getWorkerConfig() {
  ...
  const hmacSecret = process.env.DATA_HMAC_SECRET?.trim();
  const adminToken = process.env.ADMIN_TOKEN?.trim();
  return { workerBase, hmacSecret, adminToken };
}

async function workerFetch(pathname, { method = "GET", body, useHmac = false } = {}) {
  const { workerBase, hmacSecret, adminToken } = getWorkerConfig();
